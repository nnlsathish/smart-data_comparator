...
function showSplitColumnsModal(side) {
    ...
    data.headers.forEach((header, index) => {
        html += `
        <label class="splitColumnItem" style="display:flex;align-items:center;padding:6px 0;gap:8px;">
            <input type="checkbox" class="split-col" value="${index}">
            ${header}
        </label>`;
    });
    ...
    overlay.innerHTML = `
        <div style="
            ...
            <h3 style="margin-top:0;">Split by Columns</h3>
            ...
            <div style="
                ...
                <button onclick="closeSplitColumnsModal()" style="padding:8px 18px; border:1px solid #ccc; border-radius:4px; background:#f5f5f5; cursor:pointer;">
                    Cancel
                </button>
...
function filterSplitColumns() {
    ...
    document
        ...
        .forEach(item => {
            item.style.display = item.innerText.toLowerCase().includes(txt)
                ? "flex"
                : "none";
        });
}
...
function splitCurrentTable() {
    ...
    document
        .querySelectorAll(".split-col:checked")
        .forEach(cb => {
    ...
    const groupedTables = groupTableByColumns(data, selectedColumns);
    const createdIndexes = [];

    Object.entries(groupedTables).forEach(([groupName, rows]) => {
        createSet(
